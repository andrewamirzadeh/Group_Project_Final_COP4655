    ...
    private fun fetchLocation() {
        ...
        task.addOnSuccessListener {
            ...
            if (it != null && radioGroupID == R.id.rb_option_Taj_Mahal) {
                ...
                txt.setText(
                    String.format(
                        "You are %.2f miles away from the Taj Mahal",
                        tajMahalDistance
                    )
                )
                //txt.setText("${it.latitude} ${it.longitude}")
            ...
            if (it != null && radioGroupID == R.id.rb_option_Eiffel_Tower) {
                ...
                val eiffelTowerDistance =
                    haversine(it.latitude, it.longitude, eiffelTowerLat, eiffelTowerLong) / 1.609344
                txt.setText(
                    String.format(
                        "You are %.2f miles away from the Eiffel Tower",
                        eiffelTowerDistance
                    )
                )
                //txt.setText("${it.latitude} ${it.longitude}")
